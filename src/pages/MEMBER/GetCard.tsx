...
const GetCard = () => {
    ...
    const [createApp, { loading: submitting }] = useMutation(CREATE_CARD_APPLICATION, {
        onCompleted: (data) => {
            if (data.createCardApplication.ok) {
                ...
                toast.success(
                    app.status === 'APPROVED'
                        ? `Success! Assigned card ${app.assignment?.cardCode}`
    ...
    const handleSubmit = () => {
        if (!meData?.me?.street?.id) return toast.error('No street assigned to your profile.');
        createApp({
            variables: {
                input: {
    ...
    // Render Loading
    if (meLoading) return <div className="min-h-screen flex items-center justify-center bg-gray-50"><div className="loader">Loading...</div></div>;
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white p-8 rounded-2xl shadow-xl max-w-md text-center">
                    ...
                    <p className="text-gray-600 mb-6">You already have an active offering card for this year. View your contributions in the dashboard.</p>
                    <button onClick={() => navigate('/member-dashboard')} className="w-full py-3 bg-gray-100 hover:bg-gray-200 rounded-xl font-semibold text-gray-700 transition-colors">Go to Dashboard</button>
                </motion.div>
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white p-8 rounded-2xl shadow-xl max-w-md text-center">
                    ...
                    <p className="text-gray-600 mb-6">Your application is currently being reviewed by the church secretary. You'll be notified once approved.</p>
                    <button onClick={() => navigate('/member-dashboard')} className="w-full py-3 bg-gray-100 hover:bg-gray-200 rounded-xl font-semibold text-gray-700 transition-colors">Return Home</button>
                </motion.div>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-gray-50 py-12 px-4">
            <div className="max-w-screen-lg mx-auto">
                ...
                <div className="text-center mb-10">
                    <h1 className="text-3xl font-extrabold text-gray-900 mb-2">Get Your Offering Card</h1>
                    <p className="text-gray-500">Join the offering program by setting your pledges and choosing your card.</p>
                </div>
                ...
                <div className="bg-white rounded-3xl shadow-xl overflow-hidden flex flex-col md:flex-row min-h-[600px]">
                    ...
                    <div className="bg-[#5E936C] text-white p-8 md:w-1/3 flex flex-col justify-between relative overflow-hidden">
                        ...
                        <div className="relative z-10">
                            <div className="text-xs uppercase tracking-widest opacity-75 mb-6">Progress</div>
                            <div className="space-y-6">
                                <div className={`flex items-center gap-4 ${step >= 1 ? 'opacity-100' : 'opacity-50'}`}>
                                    ...
                                    <span className="font-semibold">Pledges</span>
                                </div>
                        ...
                        <div className="relative z-10 mt-12 bg-[#4a7a58] bg-opacity-50 p-4 rounded-xl backdrop-blur-sm">
                            <p className="text-sm opacity-90 italic">"Give, and it will be given to you. Good measure, pressed down, shaken together, running over..."</p>
                            <p className="text-xs mt-2 font-bold">— Luke 6:38</p>
                        </div>
                    </div>
                    ...
                    <div className="p-8 md:w-2/3 flex flex-col">
                        <div className="flex-1">
                            {step === 1 && (
                                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                                    <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-3"><FaMoneyBillWave className="text-[#5E936C]" /> Define Your Pledges</h2>
                                    <p className="text-gray-500">Enter the annual amounts you pledge to contribute for each category.</p>
                                    ...
                                    <div className="grid gap-6">
                                        {['Ahadi', 'Shukrani', 'Majengo'].map((type) => (
                                            <div key={type} className="bg-gray-50 p-4 rounded-xl border border-gray-100 hover:border-[#5E936C] transition-colors">
                                                <label className="block text-sm font-bold text-gray-700 uppercase mb-2">{type}</label>
                            ...
                            {step === 2 && (
                                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                                    <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-3"><FaCreditCard className="text-blue-600" /> Choose Your Card</h2>
                                    <p className="text-gray-500">Your street is <strong>{meData?.me?.street?.name}</strong>. Enter your preferred number below.</p>

                                    <div className="space-y-1">
                                        <label className="text-sm font-semibold text-gray-700">Preferred Number</label>
                                        <div className="relative">
                                            <input
                                                type="number"
                                                value={cardNumber}
                                                ...
                                                placeholder="e.g. 50"
                                            />
                                            {suggestionLoading && <span className="absolute right-4 top-1/2 -translate-y-1/2 text-sm text-blue-500">Checking...</span>}
                                        </div>
                                    </div>
                                    ...
                                    {debouncedNumber && suggestionData?.numberSuggestions && (
                                        <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className={`p-4 rounded-xl border ${suggestionData.numberSuggestions.exactAvailable ? 'bg-[#E8FFD7] border-[#93DA97] text-[#5E936C]' : 'bg-red-50 border-red-200 text-red-800'}`}>
                                            {suggestionData.numberSuggestions.exactAvailable ? (
                                                <div className="flex items-center gap-2 font-bold"><FaCheckCircle /> Available!</div>
                                            ) : (
                                                <div className="space-y-2">
                                                    <div className="flex items-center gap-2 font-bold"><FaExclamationCircle /> Taken or Reserved</div>
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <textarea
                                            ...
                                            placeholder="Any special requests regarding this application..."
                                        ></textarea>
                                    </div>
                                </motion.div>
                            )}
                            ...
                            {step === 3 && (
                                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                                    <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-3">Review & Submit</h2>
                                    <div className="bg-gray-50 rounded-xl p-6 space-y-4 border border-gray-100">
                                        ...
                                        <div className="flex justify-between border-b border-gray-200 pb-2">
                                            <span className="text-gray-500">Requested Number</span>
                                            <span className="font-semibold">#{cardNumber || 'Any'}</span>
                                        </div>
                                        <div className="pt-2">
                                            <span className="text-gray-500 block mb-2">Total Pledges</span>
                        ...
                        <div className="pt-8 border-t border-gray-100 flex justify-between mt-auto">
                            ...
                            {step < 3 ? (
                                ...
                                >
                                    {submitting ? 'Submitting...' : 'Confirm Application'} <FaCheckCircle />
                                </button>
                            )}
                        </div>
                    </div>
